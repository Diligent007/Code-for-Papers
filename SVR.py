...
for b in range(0, n):
    ...
    for mod in mods:
        ...
        output = csv.writer(open('label_cross_predict_%s_%s_pca%d_pc%d_交叉训练集%d.csv' % (name, mod, n, b+1, a), 'a', newline=''), dialect='excel')
    ...
    output = csv.writer(open('label_cross_%s_pca%d_pc%d_交叉训练集%d.csv' % (name, n, b+1, a), 'a', newline=''), dialect='excel')
    output.writerows(map(lambda x: x, label_cross))
    output = csv.writer(open('label_test_%s_pca%d_pc%d_交叉训练集%d.csv' % (name, n, b+1, a), 'a', newline=''), dialect='excel')
